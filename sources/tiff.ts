...
type TIFFFieldNames = 'Artist' |
	...
	// Photoshop
	'Photoshop';
type TIFFFieldDictionary = {
[K in TIFFFieldNames]: { type: string, values: (string | number)[] }
}
...
class TIFFParser {
	...
	getFieldValues(fieldTagName: string, fieldTypeName: string, typeCount: number, valueOffset: number) {
		...
		if (fieldTypeName === 'ASCII') {
			fieldValues.forEach(function (e, i, a) { a[i] = String.fromCharCode(e as number); });
		}
	...
	parseTIFF(tiffArrayBuffer: ArrayBuffer, canvas: HTMLCanvasElement) {
		...
		if (fileDirectory.StripByteCounts) {
			var stripByteCountValues = fileDirectory.StripByteCounts.values as number[];
		} else {
